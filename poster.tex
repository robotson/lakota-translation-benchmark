...
% A little extra padding inside blocks (default sep=2ex feels cramped at A0)
\setbeamertemplate{block begin}{%
  \begin{beamercolorbox}[colsep*=.75ex,sep=14pt,rounded=true]{block title}%
    \usebeamerfont*{block title}\insertblocktitle%
  \end{beamercolorbox}%
  \nointerlineskip\vskip-0.5pt%
  \begin{beamercolorbox}[colsep*=.75ex,sep=14pt,rounded=true,vmode]{block body}%
}
...
\centering
{\fontsize{64}{92}\selectfont\headfont\bfseries No frontier LLM reliably translates {\normalfont\rmfamily\bfseries\itshape\color{accent}\fontsize{82}{92}\selectfont Lakȟótiyapi}.}\\[14pt]
...
\item \textbf{Metrics:} chrF++; diacritic-normalized chrF++; an LLM \emph{semantic} judge on the English side.
\end{itemize}
\end{block}

\begin{block}{LLM-as-judge for L\,$\rightarrow$\,E}
chrF++ measures character and word overlap, but it can reward plausible nonsense. We wanted an automated measure that could take meaning into account. We use an LLM-as-judge: for the L\,$\rightarrow$\,E direction the output is English, so a model can assess whether each translation carries the reference's meaning. To keep the judge from favoring its own family, every pair is scored by two LLMs from different families, Gemini 3 Flash and an independent Llama-3.3-70B. They agree substantially (Cohen's $\kappa=0.75$, 0.89 quadratic-weighted), so the judgments are cross-validated rather than one model's unchecked read.
\end{block}

\begin{block}{Standard Lakota Orthography}
\begin{center}{\Large \lak{č}\,\lak{š}\,\lak{ž}\,\lak{ǧ}\,\lak{ȟ}\quad\lak{á}\,\lak{é}\,\lak{í}\,\lak{ó}\,\lak{ú}\quad\lak{ŋ}\quad\lak{ʼ}}\end{center}
SLO encodes phonemic distinctions with diacritics. To account for variance in orthographic conventions, we re-score with the marks stripped: the gain is only +3.3--6.5 chrF++ (E\,$\rightarrow$\,L), small enough to suggest the marks aren't the main error these models make.
\end{block}
...
\begin{axis}[
  ...
  ytick={1,2,3,4,5,6,7},
  yticklabels={Gemini, Opus, Sonnet, GPT-5.2, DeepSeek, Qwen, GLM},
...
\addplot[gray!55, line width=2pt, mark=none] coordinates {(26.1,6) (7.1,6)};
\addplot[gray!55, line width=2pt, mark=none] coordinates {(38.0,5) (19.0,5)};
...
\addplot[only marks, mark=*, mark options={fill=steel,draw=steel}, mark size=5pt]
  coordinates {(59.4,1)(45.9,2)(46.1,3)(30.0,4)(26.1,6)(38.0,5)(29.3,7)};

% % equivalent dots (terracotta)
\addplot[only marks, mark=*, mark options={fill=accent,draw=accent}, mark size=5pt]
  coordinates {(61.3,1)(37.0,2)(30.7,3)(13.4,4)(7.1,6)(19.0,5)(9.2,7)};
...
\node[anchor=west, font=\small, color=steel, xshift=8pt] at (axis cs:38.0,5) {38.0};
\node[anchor=east, font=\small, color=accent, xshift=-8pt] at (axis cs:19.0,5) {19.0};
\node[anchor=west, font=\small, color=steel, xshift=8pt] at (axis cs:26.1,6) {26.1};
\node[anchor=east, font=\small, color=accent, xshift=-8pt] at (axis cs:7.1,6) {7.1};
...
\begin{block}{Phrasebook concentration (L\,$\rightarrow$\,E)}
\centering\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Model} & \textbf{Sc.\,4} & \textbf{Sc.\,5} & \textbf{Sc.\,6} & \textbf{$\Delta$} \\
\midrule
Gemini 3.1 Pro & 52.7 & 58.2 & \textbf{81.9} & +29.2 \\
...
\end{tabular}
\\[8pt]
{\footnotesize chrF++ stratified by NLD conversational score: 4 = more formal/complex, 6 = most conversational. Every model lifts sharply at 6 --- performance concentrates on high-frequency phrasebook-like items rather than reflecting general capability.}
\end{block}
\end{column}
\begin{column}{0.49\textwidth}
...
\centering
\begin{minipage}{0.34\linewidth}\centering\qrcode[height=4.5cm,version=4]{https://github.com/robotson/lakota-translation-benchmark}\end{minipage}\hfill
\begin{minipage}{0.62\linewidth}\raggedright
{\headfont\bfseries Aggregated results and eval harness code}\\[3pt]
{\small github.com/robotson/\\lakota-translation-benchmark}\\[4pt]
{\footnotesize (language data withheld from open web --- community corpus)}
\end{minipage}
\end{beamercolorbox}
\end{column}
\end{columns}
