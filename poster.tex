...
\begin{tikzpicture}
\begin{axis}[
  ybar, bar width=22pt, width=0.62\paperwidth, height=24cm,
  ...
  xtick=data, x tick label style={font=\normalsize},
  ytick={0,20,40,60,80}, tick label style={font=\small},
  ymajorgrids=true, grid style={dashed,gray!35,thin},
  legend style={at={(0.5,1.04)},anchor=south,legend columns=2,font=\large,draw=none,fill=none},
  axis lines=left,
  nodes near coords, every node near coord/.append style={font=\footnotesize,color=ink,/pgf/number format/.cd,fixed,fixed zerofill,precision=1},
]
...
\end{tikzpicture}
\\[8pt]
{\footnotesize The picture shows the divergence; the tables below prove it. chrF++ stays in the 30s--40s even where genuine semantic equivalence collapses below 15\%. The two metrics agree for the strongest model and diverge sharply for the rest --- so chrF++ alone is misleading for low-resource translation.}
\end{block}
