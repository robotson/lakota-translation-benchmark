...
We evaluate four frontier large language models on bidirectional Lakota–English translation using 200 sentence pairs from the New Lakota Dictionary. Two experimental conditions are compared: a baseline with near-deterministic decoding and no extended thinking, and a second condition with extended thinking enabled at maximum provider settings. The best model (Gemini 3.1 Pro) achieves a mean chrF++ of 59.4 on Lakota→English and 42.6 on English→Lakota. Extended thinking improves scores by 1–7 chrF++ points. No model produces reliable translation in either direction. Diacritic normalization analysis shows models have partial structural knowledge of Lakota morphology but lack orthographic precision. All results and evaluation code are publicly available at https://github.com/robotson/lakota-translation-benchmark.
...
Stripping diacritics adds 3.6–6.5 chrF++ points for E→L. The delta is 0.0 for all models on L→E (English has no comparable diacritical system), confirming that normalization behaves as a control.
...
The highest chrF++ score in this evaluation — Gemini's 59.4 on Lakota→English — roughly corresponds to capturing the general topic of a sentence while missing specifics. For English→Lakota, no model exceeds 42.6. These scores are inadequate for any use case requiring accurate translation.
